from data.data_loader_ms import Dataset_ETT_hour
from exp.exp_basic_ms import Exp_Basic
from mindspore.dataset import GeneratorDataset
from models.model_ms import Informer

from utils.tools_ms import EarlyStopping
...
class Exp_Informer(Exp_Basic):
    ...
    def vali(self, model, vali_data, vali_source, criterion):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_data.create_tuple_iterator()):
            pred, true = self._process_one_batch(
                model, vali_source, batch_x, batch_y, batch_x_mark, batch_y_mark)
    ...
    def test(self, model, setting):
        ...
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_data.create_tuple_iterator()):
            pred, true = self._process_one_batch(
                model, test_source, batch_x, batch_y, batch_x_mark, batch_y_mark)
    ...
    def _process_one_batch(self, model, dataset_object, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        if self.args.output_attention:
            res = model(batch_x, batch_x_mark, dec_inp, batch_y_mark)
